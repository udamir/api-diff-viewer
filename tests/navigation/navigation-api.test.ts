...
describe("goToNextChange", () => {
	...
	it("returns null when no changes", () => {
		const { nav } = createNavFromSpecs({ info: { title: "same" } }, { info: { title: "same" } });
...
describe("goToPath", () => {
	...
	it("does nothing for non-existent path", () => {
		const { nav } = createNavFromSpecs({ key: "value" }, { key: "changed" });
...
describe("findPaths", () => {
	...
	it("finds values matching search text", () => {
		const { nav } = createNavFromSpecs({ info: { title: "Pet Store" } }, { info: { title: "Pet Shop" } });
	...
	it("respects limit option", () => {
		const { nav } = createNavFromSpecs({ info: { a: "x", b: "x", c: "x" } }, { info: { a: "y", b: "y", c: "y" } });

		// Search for a term that matches multiple keys
		const _allResults = nav.findPaths("info", { searchIn: "keys" });
		const limitedResults = nav.findPaths("info", { searchIn: "keys", limit: 1 });
		expect(limitedResults.length).toBeLessThanOrEqual(1);
	});

	it("is case-insensitive by default", () => {
		const { nav } = createNavFromSpecs({ Title: "Hello" }, { Title: "World" });
...
describe("getChildKeys", () => {
	...
	it("reports hasDirectChange for changed keys", () => {
		const { nav } = createNavFromSpecs({ info: { title: "Old" } }, { info: { title: "New" } });
	...
	it("returns path in encoded string form", () => {
		const { nav } = createNavFromSpecs({ paths: { "/pets": { get: {} } } }, { paths: { "/pets": { get: {} } } });
...
describe("getChangeSummary", () => {
	it("counts changes correctly", () => {
		...
		expect(summary.total).toBe(summary.breaking + summary.nonBreaking + summary.annotation + summary.unclassified);
	});
