...
import { DiffBlockData, Token, type TokenTag } from "../../src/diff-builder/common";
import {
 ...
 */
function makeLine(index: number, indent: number, tokens: Token[], diff?: DiffMeta, id = ""): DiffBlockData {
	const block = new DiffBlockData(index, indent, tokens, diff, id);
	return block;
}

function makeToken(value: string, tags?: TokenTag[]): Token {
...
function createTestBlocks(): DiffBlockData[] {
	...
	// Unchanged line
	root.addBlock(makeLine(1, 0, [makeToken("key: value")]));
	// Added line
	root.addBlock(makeLine(2, 0, [makeToken("newKey: newValue")], { action: "add", type: "non-breaking" }, "add1"));
	// Removed line
	root.addBlock(makeLine(3, 0, [makeToken("oldKey: oldValue")], { action: "remove", type: "breaking" }, "rm1"));
	// Modified/replaced line
	root.addBlock(
		makeLine(
			4,
			0,
			[makeToken("key: ", []), makeToken("before", ["before"]), makeToken("after", ["after"])],
			{ action: "replace", type: "non-breaking" },
			"rep1"
		)
	);
...
function createBlockWithAction(action: string): DiffBlockData[] {
	const root = new DiffBlockData(1, -2, []);
	const diff = { action, type: "non-breaking" } as DiffMeta;

	root.addBlock(
		makeLine(
			1,
			0,
			[makeToken("key: ", []), makeToken("old", ["before"]), makeToken("new", ["after"])],
			diff,
			`${action}1`
		)
	);
