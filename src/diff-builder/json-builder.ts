...
import type { MergedDocument, MergedObject } from "../types";
import { encodeKey, isEmpty } from "../utils/common";
import { DiffBlockData, metaKey, Token, type TokenTag } from "./common";

export const buildDiffJson = (input: MergedDocument, parent: DiffBlockData) => {
...
export const addJsonBlockTokens = (block: DiffBlockData) => {
	let _added = block.children.length;
	let _removed = block.children.length;

	block.children.forEach((child) => {
		_added -= child.diff?.action === "add" ? 1 : 0;
		_removed -= child.diff?.action === "remove" ? 1 : 0;
	});

	if (block.tokens.length) {
		const tokens =
			block.diffs?.map((c, i) => c && Token.Change(c, i, "collapsed")).filter((v): v is Token => !!v) || [];
		block.tokens.push(...tokens);
	}
};

export const _jsonValueTokens = (
	tokenConstrucor: (value: string, tags?: TokenTag | TokenTag[]) => Token,
	value: unknown,
	diff?: DiffMeta
) => {
...
export const _jsonPropLineTokens = (key: string | number, value: unknown, diff?: DiffMeta, last = false) => {
	return [
...
export const _jsonArrLineTokens = (value: unknown, diff?: DiffMeta, last?: boolean) => {
	return [..._jsonValueTokens(Token.Value, value, diff), ...(last ? [] : [Token.Spec(",")])];
};
...
export const _jsonEndBlockTokens = (isArray: boolean, last: boolean) => {
	return [...(isArray ? [Token.Spec(`]${last ? "" : ","}`)] : [Token.Spec(`}${last ? "" : ","}`)])];
};

export const buildDiffJsonBlock = (input: MergedDocument, key: string | number, parent: DiffBlockData, last: boolean) => {
	const value = (input as Record<string | number, unknown>)[key];
	const meta = metaKey in input ? (input as MergedObject).$diff : undefined;
	let diff: DiffMeta | undefined = meta?.[String(key)];
	...
	if (typeof value !== "object" || input instanceof Date || isEmpty(value)) {
		...
		buildDiffJson(value as MergedDocument, block);
		block.addBlock(
			new DiffBlockData(block.nextLine, indent + 2, _jsonEndBlockTokens(Array.isArray(value), last), diff)
		);
	}
