...
export const Line = ({ index, indent, tokens, diff, tags }: LineProps) => {
  ...
  const content = tokens.filter((token) => token.tags.every((v) => tags.includes(v))).map((token, i) => 
    diffTypes.includes(token.type as any) 
      ...
      : <span key={i} className={`token ${token.type} ${token.tags || ""}`}>{ token.value }</span>
  )

  const lineClass = `line${right ? " right non-selectable" : ""}${hidden ? " bg-hidden" : diff?.action ? " bg-" + diff?.action : ""}`
  ...
  return (
    <div className={lineClass}>
      ...
      <div className={`line-content${hidden ? " hidden": ""}`} style={contentIndentStyle}>
