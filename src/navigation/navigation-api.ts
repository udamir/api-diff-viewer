...
import type { DiffType, DiffMeta } from 'api-smart-diff'
...
export class NavigationAPIImpl implements NavigationAPI {
  ...
  goToNextChange(...types: DiffType[]): string | null {
    const changedBlocks = types.length > 0
      ? this.getBlocksByType(types)
  ...
  getChildKeys(path?: DiffPath): ChildKeyInfo[] {
    ...
    const node = segments.length > 0
      ? getPathValue(this.merged as Record<string, unknown>, segments)
      : this.merged
