...
import type { DiffData, DiffConfig, LineMapping, BlockMapping } from '../types'
...
export const diffConfigFacet = Facet.define<Partial<DiffConfig>, DiffConfig>({
  combine(values): DiffConfig {
    ...
    return values.reduce<DiffConfig>((acc, val) => ({ ...acc, ...val }), defaultConfig)
  },
})
...
export function getDiffConfig(state: { facet: <T>(facet: Facet<any, T>) => T }): DiffConfig {
