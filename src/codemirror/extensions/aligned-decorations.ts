...
import { type Extension, type RangeSet, RangeSetBuilder, StateEffect, StateField } from "@codemirror/state";
import {
	Decoration,
	type DecorationSet,
	EditorView,
...
function buildDiffGutterMarkers(
	...
	for (let i = 0; i < numLines; i++) {
		...
		const sourceLineNum =
			side === "unified" ? (m.afterLine ?? m.beforeLine) : side === "before" ? m.beforeLine : m.afterLine;
...
export function createSpacerAwareLineNumbers(mappings: LineMapping[], side: "before" | "after" | "unified"): Extension {
	// Pre-calculate display numbers and change types for each document line
	const lineInfo = new Map<number, { displayNum: number | null; changeType: GutterChangeType }>();
