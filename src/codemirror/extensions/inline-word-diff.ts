...
function computeLineDecorations(
	...
	const changes = diffFn(beforeContent, afterContent);
	...
	for (const lineData of sortedData) {
		if (lineData.lineNumber > 0 && lineData.lineNumber <= doc.lines) {
			try {
				...
				computeLineDecorations(line.from, lineData.beforeContent, lineData.afterContent, config.mode, builder);
			} catch {
				// Line doesn't exist
			}
		}
		...
		update(update: ViewUpdate) {
			if (
				...
				update.transactions.some((tr) =>
					tr.effects.some((e) => e.is(setInlineWordDiffEffect) || e.is(setInlineWordDiffConfigEffect))
				)
			) {
